...
- base_ref, head_ref: full refs in `refs/heads/<tail>` form, each tail
  validated as in Stage 21A section 7.1; any other form is rejected before
  collection;
...
- FAILED is tolerated, and dispatch continues, iff all four hold: the slot is
  ...
  its reason codes are exactly [HTTP_RESPONSE_REJECTED]; and the observation has
  at least one per-page record, every one of which carries a Stage 21A section 9
  safe rate-limit projection whose retry_after_present is false and whose
  remaining is non-null and greater than zero. Zero per-page records, an absent
  projection or a null remaining fails the fourth condition. A tolerated FAILED
...
records only as HTTP_RESPONSE_REJECTED but that does not meet the fourth
condition also stops. Beyond that, this profile does not verify Stage 21A's rate-limit
...
observation has status class 2xx and a valid retained projection showing an
actual difference from the bound repository profile: for repository or
repository_after, its id differs from the bound repository ID or its owner.id
from the bound account ID; for object_format or object_format_after, its
hash_algorithm differs from the profile's git_object_format. An
identity_match false without such a projection — for example after a timeout,
a 404 or an invalid projection — is not evidence of change. Otherwise target is
UNKNOWN.
...
target. For report derivation, expected_base_sha is used only by section 5.3;
section 3's request-equality check is unaffected. Equal endpoints do not
...
record validity under section 3, presence, bindings, attempt-digest non-use and
the section 6 budget only; it evaluates no authorization or intent time
validity. Stage 21A evaluates time validity at
...
- section 4.4 continuation: a protection observation with status class 4xx and
  ...
  retry_after_present true, with remaining zero, with remaining null, with no
  rate-limit projection, and with zero per-page records, each stopping; that response combined with
...
- section 4.5 target: SAME with all four verification slots OBSERVED; SAME in
  ...
  differs from expected_head_sha; CHANGED from a verification slot whose 2xx
  projection shows a different repository ID, account ID or object format;
  UNKNOWN, not CHANGED, from a verification slot that timed out or returned 404;
  request refs compared after stripping `refs/heads/`; a request ref not in
  `refs/heads/<tail>` form, such as `refs/tags/x`, rejected before collection;
