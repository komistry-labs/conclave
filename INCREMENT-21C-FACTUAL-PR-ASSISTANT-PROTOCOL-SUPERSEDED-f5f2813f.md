...
4. Retain administration-read-only restrictions, Stage 21A/21B controls,
   ...
   For this factual profile only, exactly the following Stage 21A clauses
   receive additive precedence, each specified in section 5:
   ...
   (b) every site at which Stage 21A validates or records an API-profile
   reference or hash, extended to accept that record, while old validators still
   reject it. These sites are sections 5.1, 5.2 (including the attempt
   preimage), 5.3, 6.3 and 9; a site not listed here is governed by this rule
   all the same;
...
- FAILED is tolerated, and dispatch continues, iff all three hold: the slot is
  protection, branch_rules or rulesets; the observation's status class is 4xx;
  and its reason codes are exactly [HTTP_RESPONSE_REJECTED]. A tolerated FAILED
...
404 means unprotected; a rate-limited, ambiguous, cleanup-failed or 5xx result
fails the exception because its code set or status class differs.
...
Both PR reads bind numeric repository/PR IDs and refs. target is SAME iff
pr_before and pr_after are both OBSERVED with base/head/ref identity equal to the
request, base_ref is OBSERVED and matching, and all four verification slots are
OBSERVED. Any explicit mismatch — a PR read reporting a changed head or base, or a
verification slot whose observation has identity_match false — is CHANGED.
Otherwise target is UNKNOWN. Equal endpoints do not prove atomicity or absence of
intermediate changes. Never retry a cycle to hide drift.

A correctly requested PR reporting a changed head/base is valid evidence of
CHANGED, not a source-binding error. Each step binds the exact request
...
insufficient. Validate all fixed-slot attempts before dispatch. For post_action,
...
repository_after. If pr_after is not OBSERVED, section 4.4 has already stopped
dispatch and the three slots are NOT_ATTEMPTED.
...
commit_source -> UNKNOWN. This is structural comparison, not proof of merge
method, approved tree or policy compliance.
...
references or storage failure returns an error, not a report. An error never
...
- plan and binding: wrong-repository/SHA reference injection; swapped PR slots;
  ...
  new projection-set/profile binding and old-validator rejection;
...
- section 4.4 continuation: a protection observation with status class 4xx and
  ...
  LINKAGE_CHANGED recorded as stop_reason; that response combined with
  ...
  retained prefix;
- section 4.5 target: SAME with all four verification slots OBSERVED; CHANGED
  from a verification slot with identity_match false; UNKNOWN when the closing
  pair is NOT_ATTEMPTED after an earlier stop; open-to-merged between reads;
  missing final read;
...
- section 5.2 linkage: post_action null merge_sha and differing merge_sha, each
  ...
  merge_commit leaving commit_source null and parents_comparison UNKNOWN;
