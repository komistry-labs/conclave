...
missing lease and provider authentication failure at a later slot after a
nonempty durable prefix, yielding ATTEMPTED_NO_OBSERVATION with a retained
claim and no fabricated observation; and each coverage-algorithm branch in turn
— initial post-merge, failed merge-commit read, null linkage and partial
checks.
