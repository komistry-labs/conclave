...
- stop_reason: NONE/TIME_BUDGET/PROFILE_EXPIRY/OPERATION_FAILURE/LINKAGE_CHANGED;
...
and V the frozen Stage 21A profile-verification expiry after the first two
successful operations. Before every later slot reserve its full 60-second
operation deadline: require now+60 <= min(D,V), as well as valid lease/authority.
Before the verification pair require now+60 <= D. No cached pair from a prior
cycle is accepted. If time cannot accommodate a whole operation, stop without
dispatch, mark remaining applicable slots NOT_ATTEMPTED, and record TIME_BUDGET
or PROFILE_EXPIRY (expiry wins a tie). No extra verification pair is inserted.
...
stopping; insufficient time before dispatch; profile expiry; retained prefix;
new projection-set/profile binding and old-validator rejection; pre-issued
merge authorization with changed final SHA; and null expected_human_id.
