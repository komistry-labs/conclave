...
(CR4 option A). This draft has no governing effect. It becomes a record only when
Arthur completes §9. Until then every instrument named below keeps its full
current effect.
...
- the factual candidate's §2 governs the frozen protocol **documents** (master
  protocol, Erratum 0001, the readiness and collection profiles);
- this record governs Arthur's merged **freeze records**, which select among
  those documents.

Neither instrument restates the other's rules. The candidate identifies this
record by path; this record binds the candidate by exact hash (§2). That is the
only hash binding between the two, and it runs in one direction.
...
- **§5 items 2, 3, 4 and 7** — deterministic exact-head readiness reduction;
  independently governed no-bypass and action-spanning control attestations;
  separately authored human-only merge-authorization evidence; factual
  reconciliation. Superseded as requirements of Stage 21C.
- **§5 items 1, 5 and 6** — bounded Stage 21A read-only observations and
  single-operation authorizations; a complete stop before the external human
  action; separately authorized read-only post-action observation. **Retained.**
  The adopted object satisfies each.
...
- **§4 items 2-5** — the human merge as a direct action outside CONCLAVE; no
  merge permission, write credential, mutation lease, merge endpoint, send point,
  retry path, merge receipt or hidden mutation route; the narrow master
  supersession; the preservation of 21A, 21B, 21B Erratum 0001, 21D and v0.8.0.
  **Retained in full.**
...
- **§2** — subordination of the collection protocol to `d3701754…`. Unaffected
  as a statement about those two frozen documents. It gives neither of them any
  selected status after this record.
- **§§5 items 1-10, 6** — the collection procedure's own content and fail-closed
  limits are unaffected as frozen text. They are unselected, not rescinded.
...
- The frozen Increment 21 master protocol, Erratum 0001, and both unselected
  profiles remain byte-identical. Supersession of the master and erratum
  **text** is governed solely by the adopted object's §2, not by this record.
- Stage 21A, Stage 21B, Stage 21B Erratum 0001, Stage 21D, v0.8.0, and all
...
To be completed by Arthur only. Until completed this record has no effect.
...
`council_standard_accepted` records Arthur's determination on the open question
carried by Council reviews 0001 and 0002: whether five same-provider subagent
seats satisfy the five-seat requirement of Erratum 0001 freeze record §6.
